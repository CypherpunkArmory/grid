...
variable "userland_deploy_version" {
  default = ""
}
...
resource "vault_generic_secret" "userland_secrets" {
...
resource "vault_generic_secret" "prod_domain_certs" {
  ...
  data_json = <<EOH
{
  "cert": ${jsonencode(acme_certificate.userlandio_certificate[0].certificate_pem)},
  "key": ${jsonencode(acme_certificate.userlandio_certificate[0].private_key_pem)},
  "chain": ${jsonencode(acme_certificate.userlandio_certificate[0].issuer_pem)}
}
EOH

}

data "template_file" "userland_policy" {
  template = file("${path.module}/templates/userland-policy.tpl.hcl")
  vars = {
    api_domain = "api.${local.api_domain}"
  }
}

resource "vault_policy" "userland_policy" {
  name   = "userland-policy"
  policy = data.template_file.userland_policy.rendered
}
...
data "template_file" "userland_hcl" {
  template = file("${path.module}/templates/userland.tpl.hcl")
  vars = {
    deploy_version = local.userland_deploy_version_default
    api_domain     = "api.${local.api_domain}"
    env_template   = data.template_file.env_file.rendered
  }
}

resource "nomad_job" "userland" {
  jobspec = data.template_file.userland_hcl.rendered
  depends_on = [
    vault_generic_secret.domain_certs,
    vault_generic_secret.prod_domain_certs,
    vault_generic_secret.userland_secrets,
  ]
}

data "template_file" "userland_hook_hcl" {
...
resource "nomad_job" "userland_hook" {
  count   = element(split(" ", var.hook), 0) != "none" ? 1 : 0
  jobspec = data.template_file.userland_hook_hcl.rendered
}
